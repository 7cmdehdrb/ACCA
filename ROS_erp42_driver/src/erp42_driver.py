...
class control:
    ...
    def vel_pid(self):
        ...
        if (
            (error < BrakeThreshold and self.linear_vel > 0)
            or (error > -BrakeThreshold and self.linear_vel < 0)
            or self.cmd_vel_msg.linear.x == 0
        ):
    ...
    def cmd_vel_to_command(self):
        ...
        if self.cmd_vel_msg.linear.x != 0 and self.linear_vel != 0:
            self.command_msg.Deg = -degrees(
                atan(wheelbase / self.linear_vel * self.cmd_vel_msg.angular.z)
            )
    ...
    def calc_vel_and_pub_odom(self):
        ...
        if self.feedback_msg.Deg != 0:
            A = wheelbase / tan(radians(abs(self.feedback_msg.Deg))) - wheeltrack / 2
        ...
        angular_vel = tan(radians(self.feedback_msg.Deg)) * linear_vel / wheelbase
        ...
        odom_broadcaster.sendTransform(
            (self.x, self.y, 0.0), odom_quat, current_time, "base_link", "odom"
        )
        ...
        odom.twist.twist = Twist(Vector3(vx, vy, 0), Vector3(0, 0, angular_vel))
...
if __name__ == "__main__":
    ...
    rospy.Subscriber("/cmd_vel", geometry_msgs.msg.Twist, driver.cmd_vel_callback)
    rospy.Subscriber("/erp42_feedback", SerialFeedBack, driver.feedback_callback)
