...
class control:
    ...
    def send_data(self, SPEED, STEER, BRAKE, GEAR):
        SPEED = 30
        STEER = math.degrees(self.dg_steer)
    ...
    def handle_data(self, line):
        ...
        self.feedback.Encoder = (
            ord(line[11])
            + ord(line[12]) * 256
            + ord(line[13]) * 256 * 256
            + ord(line[14]) * 256 * 256 * 256
        )
...
if __name__ == "__main__":
    ...
    rospy.Subscriber("/steer_pub", Float32, mycar.steerCallback)
    rate = rospy.Rate(50)
