...
class ConeLoad(LoadPose):
    def __init__(self):
        super(ConeLoad, self).__init__(file_name=None)
...
class ConeTracker(object):
    def __init__(self, load=ConeLoad()):
    ...
    def loopCloseCallback(self, msg):
        data = True if msg.data == 1 else False
        if data != self.isLoopClose:
            pure_pursuit.resetPath()
            print("LOOP IS CLODED!!!")

        # if data is True:
        
        self.isLoopClose = data


class PurePursuit(object):
    def __init__(self, state, cmd_msg, cmd_pub, load):
    ...
    def main(self):
        ...
        last_idx = len(self.load.cx) - 1
        if self.target_idx >= last_idx - 1:
            self.target_idx = last_idx - 1

        target_idx = self.target_idx
        di, target_idx = self.stanley.stanley_control(
            self.state, self.load.cx[0:self.target_idx+30], self.load.cy[0:self.target_idx+30], self.load.cyaw[0:self.target_idx+30], target_idx)

        self.target_idx = target_idx
        di = np.clip(di, -math.radians(30.0), math.radians(30.0))

        print(self.target_idx, last_idx)

        if self.target_idx == len(self.load.cx) - 1:
        ...
        # di = self.set_steering()loopClosure
...
if __name__ == '__main__':
    ...
    cone_path = ConeLoad()
    load = cone_path
    cone_tracker = ConeTracker(load=load)
