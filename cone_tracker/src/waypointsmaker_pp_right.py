...
SPEED = rospy.get_param("/cone_speed", 8.0)
CONE_OFFSET = rospy.get_param("/cone_offset", 1.35)
    ...
    def Generation_waypoints(self, maps):
        ...
        if len(maps) < 2:
            ...
            for i in range(len(maps)):
                ...
                dist = np.hypot(maps[i].x - self.state.x,
                                maps[i].y - self.state.y)
            ...
            data1 = [maps[min1_index].x + CONE_OFFSET * unit[0],
                     maps[min1_index].y+CONE_OFFSET * unit[1]]
            data2 = [maps[min2_index].x + CONE_OFFSET * unit[0],
                     maps[min2_index].y+CONE_OFFSET * unit[1]]
    ...
    def set_steering(self):
        if self.cx:
            ...
            if self.Ld > 2.0:
                ...
                # + self.state.yaw
                alpha = m.atan((self.cy[1] - self.state.y) /
                               (self.cx[1] - self.state.x)) - self.state.yaw
                # alpha = m.asin(dy/self.Ld)  # if error -- change dx to dy
