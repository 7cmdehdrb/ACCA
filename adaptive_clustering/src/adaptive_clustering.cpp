...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr &ros_pc2_in)
{
  ...
  if (print_fps_)
    if (++frames > 10)
    {
      std::cerr << "[adaptive_clustering] fps = " << float(frames) / (float(clock() - start_time) / CLOCKS_PER_SEC) << ", timestamp = " << clock() / CLOCKS_PER_SEC << std::endl;
      reset = true;
    } //fps
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber int_sub = nh.subscribe<std_msgs::Int32>("hdl_state", 1, stateCallback);
